...
def find_worlds(target_id: str | None = None, force: bool = False) -> list[str]:
    worlds = []
...
def main():
    ...
    parser.add_argument(
        ...
        help="Regenerate maps even if map.png already exists (overwrites)"
    )
    ...
    worlds = find_worlds(target_id=args.world_id, force=args.force)
    total_worlds = sum(1 for _ in WORLDS_DIR.glob("*.md"))
    label = "to regenerate" if args.force else "missing map.png"
    print(f"Worlds in archive: {total_worlds}")
    print(f"Worlds {label}:   {len(worlds)}")
    ...
    to_process = worlds[: args.limit] if args.limit else worlds
