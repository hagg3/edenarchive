...
export type MapPixels = { data: Uint8ClampedArray; width: number; height: number };

export function renderNormalMap(world: WorldData): MapPixels {
      ...
      for (let ly = 0; ly < 16; ly++) {
        ...
        for (let band = meta.numBands - 1; band >= 0 && !found; band--) {
          for (let lz = 15; lz >= 0; lz--) {
            ...
            const pi = bi + 4096;
            if (bi >= bytes.length || pi >= bytes.length) continue;
            ...
            const c =
              paint !== 0
                ? (Painted[paint - 1] ?? { r: 0, g: 0, b: 0 })
                : (Unpainted[block - 1] ?? { r: 0, g: 0, b: 0 });
            data[off] = c.r;
            data[off + 1] = c.g;
            data[off + 2] = c.b;
            data[off + 3] = 255;
