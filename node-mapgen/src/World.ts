...
const MAX_CHUNK_COUNT = 2_000_000;
...
const MAX_CANVAS_PIXELS = 400_000_000;
...
const MAX_CHUNKS_FOR_CLUSTERING = 200_000;
...
export function loadWorldFromArrayBuffer(buffer: ArrayBuffer): WorldData {
  ...
  let rawSpawnX: number | null = null;
  let rawSpawnZ: number | null = null;
  if (bytes.length >= 28) {
    ...
    if (address > 0 && address < bytes.length) {
      ...
      if (chunks.length > MAX_CHUNK_COUNT) {
        throw new WorldParseError(
          `chunk pointer table scan found over ${MAX_CHUNK_COUNT.toLocaleString()} ` +
  ...
  if (chunks.length === 0) {
    ...
    throw new WorldParseError(
      'no valid chunks found — this file is either not a real Eden world or its data is corrupt'
    );
  }
  ...
  if (droppedOutliers > 0) {
    console.error(
      `⚠ dropped ${droppedOutliers} outlier chunk record(s) far from the main ` +
        'cluster (likely misread bytes past the real chunk table, not real chunks)'
    );
  }
  if (worldArea.width * worldArea.height > MAX_CHUNK_AREA) {
    throw new WorldParseError(
      `world area ${worldArea.width}×${worldArea.height} chunks is implausibly large ` +
...
function boundingBox(chunks: ChunkPointer[]): WorldArea {
...
function unionBBox(a: WorldArea, b: WorldArea): WorldArea {
...
function clusterByProximity(chunks: ChunkPointer[], radius: number): number[][] {
...
function computeArea(chunks: ChunkPointer[]): {
  area: WorldArea;
  chunks: ChunkPointer[];
  ...
  let area: WorldArea = primary.bbox;
  const kept: ChunkPointer[] = [...primary.members];
